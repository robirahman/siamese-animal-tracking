...
=======
    optimizer = tf.keras.optimizers.Adam(learning_rate=1e-4)  # DLiske the orig code had lr instead of learning_rate
    loss_func = tfa.losses.TripletSemiHardLoss()  # DLiske: The triplets are generated by TF
>>>>>>> Stashed changes
...
<<<<<<< Updated upstream
=======
    model.save(f"models/{tortoise_number}")
...
if __name__ == "__main__":
    for i in [0, 1]:  # range(24)
        embed_missing_tortoise(tortoise_number=i)
>>>>>>> Stashed changes
